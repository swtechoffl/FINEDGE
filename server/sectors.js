...
// Map), each populated with a hand-picked set of representative NSE-listed
// stocks. NSE's basic equity list (EQUITY_L.csv) has no industry field, so
// this mapping was curated from general knowledge and then spot-checked
// against the live NSE symbol master to catch wrong/renamed tickers —
// that check caught 4 real mistakes: Tata Motors is now listed as TMCV (not
// TATAMOTORS), Gujarat Mineral Development Corp is GMDCLTD (not GMDC),
// Firstsource Solutions is FSL (not FIRSTSOURCE), and LTIMindtree wasn't in
// the fetched list under any obvious symbol, so it was dropped rather than
// risk a wrong ticker.
...
export const ALL_STOCKS = SECTORS.flatMap((s) => s.stocks.map((st) => ({ ...st, sector: s.name })));
